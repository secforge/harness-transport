...
func TestValidAddress(t *testing.T) {
	ok := []string{"uds:/run/user/0/cc-socks/1.sock", "bridge:abc", "did:key:z6Mk"}
	for _, a := range ok {
		if !ValidAddress(a) {
			t.Errorf("ValidAddress(%q) = false, want true", a)
		}
	}
	bad := []string{"", "uds:", "/run/user/0/x.sock", "http:/x", "uds:" + strings.Repeat("x", 201)}
	for _, a := range bad {
		if ValidAddress(a) {
...
func TestParseUDS(t *testing.T) {
	...
	if _, ok := ParseUDS("bridge:x"); ok {
		t.Error("bridge address should not parse as uds")
	}
}

func TestValidSocketName(t *testing.T) {
	ok := []string{"5543.sock", "5543-a1b2c3d4.sock", "deadbeef.sock", "0123456789abcdef.sock"}
	for _, n := range ok {
		if !ValidSocketName(n) {
			t.Errorf("ValidSocketName(%q) = false, want true", n)
		}
	}
	bad := []string{"5543.socket", "5543-A1B2C3D4.sock", "5543-a1b2.sock", "x.sock",
		"01234567890abcdef1.sock", ".sock", "5543.sock.tmp"}
	for _, n := range bad {
		if ValidSocketName(n) {
...
func TestPIDFromSocketName(t *testing.T) {
	...
	// A 16-hex opaque id carries no pid, and must not be read as one.
	if _, ok := PIDFromSocketName("deadbeefdeadbeef.sock"); ok {
		t.Error("opaque id should not yield a pid")
	}
}
...
func TestResolveReplyAddrRejectsOutsideNamespace(t *testing.T) {
	...
	if _, err := ResolveReplyAddr("bridge:x"); err == nil {
